...
export default function AvailbilityGridBResponseFilterButton({
  ...
  return (
    <motion.button
      className={cn(
        "inline-flex w-min flex-row items-center rounded-md border-2 border-purple-200 bg-purple-100 px-1 py-[1.5px] outline-none duration-100 hover:bg-purple-200",
        isFilterApplied && {
        ...
        isEditMode(mode) && {
          ...
          "border-transparent font-medium text-secondary no-underline hover:bg-purple-200": name === user
        },
        {
    ...
    >
      ...
      <span className="mx-1 max-w-[5.8rem] overflow-hidden text-ellipsis whitespace-nowrap text-[0.74rem]">{name}</span>
    </motion.button>
  );
}
