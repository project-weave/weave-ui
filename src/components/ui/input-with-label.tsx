...
export default function InputWithLabel({ id, label, type, ...props }: InputWithLabelProps) {
  return (
    <div className="relative w-full">
      ...
      />
      <Label
        className="absolute left-1 top-1.5 z-10 origin-[0] -translate-y-4 scale-75 transform rounded-sm bg-background px-2 text-sm text-black duration-300 peer-placeholder-shown:top-1/2 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:scale-100 peer-focus:top-1.5 peer-focus:-translate-y-4 peer-focus:scale-75 peer-focus:px-2"
        htmlFor={id}
      >
        {label}
      </Label>
