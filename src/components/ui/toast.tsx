...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    className={cn(
      "fixed right-0 top-0 z-[999] flex max-h-screen w-full max-w-[20rem] flex-col-reverse p-4 md:bottom-0 md:top-auto md:max-w-[420px] md:flex-col",
      className
    )}
    ref={ref}
    {...props}
...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border py-4 sm:py-6 pl-4 sm:pl-6 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    defaultVariants: {
      variant: "default"
    },
    variants: {
      variant: {
        ...
        failure: "failure group border-failure bg-failure-light text-failure-foreground",
        success: "success group border-success bg-success-light text-success-foreground"
      }
    }
  }
);
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Title className={cn("hidden font-medium sm:block", className)} ref={ref} {...props} />
));
